  ...
  protected parseFrames(view: AMQPView): void {
    ...
    for (let i = 0; i < view.byteLength; ) {
      ...
      try {
        ...
        if (frameEnd !== 206)
          throw new AMQPError(
            `Invalid frame end ${frameEnd}, expected 206`,
            this,
          )
      } catch (e) {
        throw new AMQPError(
      ...
      switch (type) {
              ...
              switch (methodId) {
                ...
                case 30: {
                  ...
                  this.channelMax =
                    this.channelMax === 0
                      ? channelMax
                      : Math.min(this.channelMax, channelMax)
                  this.frameMax =
                    this.frameMax === 0
                      ? frameMax
                      : Math.min(this.frameMax, frameMax)
                  this.heartbeat =
                    this.heartbeat === 0
                      ? 0
                      : Math.min(this.heartbeat, heartbeat)
                ...
                case 50: {
                  ...
                  j += 1
                  this.send(new Uint8Array(closeOk.buffer, 0, j)).catch(
                    (err) =>
                      this.logger?.warn(
                        "Error while sending Connection#CloseOk",
                        err,
                      ),
                  )
                  this.onerror(err)
              ...
              switch (methodId) {
                ...
                case 40: {
                  ...
                  this.send(new Uint8Array(closeOk.buffer, 0, j)).catch(
                    (err) =>
                      this.logger?.error(
                        "Error while sending Channel#closeOk",
                        err,
                      ),
                  )
                  break
        ...
        case 8: {
          ...
          this.send(heartbeat).catch(
            (err) => this.logger?.warn("Error while sending heartbeat", err),
          )
          break
        }
        default:
